...
func (c *clientState) Challenge(exitingTx *chain.ConfirmedTransaction, exitingOutput uint8, exitingDepositNonce *big.Int, challengingTx *chain.ConfirmedTransaction, proof []byte, confirmSigs chain.Signature) (*types.Receipt, error) {
	opts := CreateKeyedTransactor(c.privateKey)
	...
	receipt, err := ContractCall(c.client, func() (*types.Transaction, error) {
		return c.contract.ChallengeExit(opts, exitingTxPos, challengingTxPos, challengingTx.Transaction.RLP(), proof, confirmSigs[:])
	})
	if err != nil {
		return nil, err
	}
